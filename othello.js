...
const blackCounter = document.getElementById("blackNum");
const whiteCounter = document.getElementById("whiteNum");
    ...
    for (let j = 1; j < 9; j++) {
      ...
      switch (squareArrays[i][j]) {
        ...
        case -1:
          ...
          break;
        default:
          console.log("squareArraysに想定外の数値が入っている");
      }
    }
  }
};
...
const checkSquare = (x, y, dx, dy) => {
  ...
  const checkNext = (x, y, dx, dy) => {
    ...
    switch (nextSquare) {
      ...
      default:
        console.log("nextSquareに想定外の数値が入っている");
    }
  };
  checkNext(x, y, dx, dy);
};
...
const playGame = (x, y) => {
  ...
  blackCounter.textContent = `黒：${blackNum}`;
  whiteCounter.textContent = `白：${whiteNum}`;
...
const getRandomNums = () => {
  const randX = Math.floor(Math.random() * 8)+1;
  const randY = Math.floor(Math.random() * 8 )+1;
  return [randX, randY];
};

const playWhite = () => {
  const whiteNums = getRandomNums();
  if (hasClickable(whiteNums[0], whiteNums[1])) {
    const target = document.getElementById(`${whiteNums[0]}${whiteNums[1]}`);
    const clickEvent = new Event("click");
    console.log("play");
    console.log(whiteNums);
    target.dispatchEvent(clickEvent);
    setTimeout(function(){playGame(whiteNums[0], whiteNums[1])}, 1500);
